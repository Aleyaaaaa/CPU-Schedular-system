...
# Non-Preemptive priority (Aleya)

@app.route('/npp', methods=['GET', 'POST'])
def pp():
    if request.method == 'POST':
        try:
            process_names = request.form['process_names'].split(',')
            ...
            if n < 3 or n > 10:
                error = "Number of processes must be between 3 and 10."
                return render_template('npp.html', error=error)
            ...
            result = npp_scheduler(n, arrival, burst, priority,process_names)
            return render_template('resultnpp.html', result=result)

        except ValueError:
            error = "Invalid input. Please enter integers for process count, arrival times, burst times, and priorities."
            return render_template('npp.html', error=error)

    return render_template('npp.html')

def npp_scheduler(n, arrival, burst, priority, process_names):
    T = [[process_names[i], arrival[i], burst[i], priority[i], 0, 0, 0] for i in range(n)]
    ...
    while completed < n:
        ...
        if ready_queue:
            ...
            if gantt_chart and gantt_chart[-1][0] == '-':
                gantt_chart[-1] = ('-', gantt_chart[-1][1], current_time)
            gantt_chart.append((idx[0], current_time, current_time + idx[2]))
